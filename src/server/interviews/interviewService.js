...
const buildResolvedContextSnapshot = (input = {}, context = {}) => ({
  mode: input.mode || context.mode || 'operating',
  modeLabel: input.modeLabel || context.modeLabel || '',
  personaId: input.personaId || context.personaId || '',
  personaLabel: input.personaLabel || context.personaLabel || '',
  personaSummary: input.personaSummary || context.personaSummary || '',
  personaPromptStyle: input.personaPromptStyle || context.personaPromptStyle || '',
  interviewerName: input.interviewerName || context.interviewerName || '',
  ttsVoice: input.ttsVoice || context.ttsVoice || '',
  ttsInstructions: input.ttsInstructions || context.ttsInstructions || '',
  gradingProfile: input.gradingProfile || context.gradingProfile || '',
  scrutinyProfile: input.scrutinyProfile || context.scrutinyProfile || '',
  silly: input.silly ?? context.silly ?? false,
  customTone: input.customTone ?? context.customTone ?? '',
  seriousness: input.seriousness ?? context.seriousness ?? 0.5,
  style: input.style ?? context.style ?? 0.5,
  difficulty: input.difficulty ?? context.difficulty ?? 0.5,
  complexity: input.complexity ?? context.complexity ?? 0.5,
  webSearchEnabled: input.webSearchEnabled ?? context.webSearchEnabled ?? true,
});
...
const startInterviewSession = async ({
  ...
  const completion = await openaiClient.chat.completions.create({
    ...
    messages: buildStartMessages({
      ...
      personaPromptStyle: input.personaPromptStyle,
      silly: input.silly,
  ...
  return {
    ...
    chatId,
    ...buildResolvedContextSnapshot(input),
    backgroundDoc: context.backgroundDoc,
...
const continueInterview = async ({
  ...
  const turnDoc = await persistInterviewTurn({
    ...
    context: {
      ...
      ttsInstructions: input.ttsInstructions,
      gradingProfile: input.gradingProfile,
      scrutinyProfile: input.scrutinyProfile,
      silly: input.silly,
      customTone: input.customTone,
      seriousness: input.seriousness,
      style: input.style,
...
const finalizeInterview = async ({
  ...
  const finalReview = await createFinalInterviewReview({
    ...
    company: chatLog.context?.company || '',
    role: chatLog.context?.role || '',
    mode: chatLog.context?.mode || 'operating',
    gradingProfile: chatLog.context?.gradingProfile || '',
    personaLabel: chatLog.context?.personaLabel || '',
